...
from src.common.database.controller.retrieval_controller import (
    RetrievalController,
    RetrievalData,
)
...
class RetrievalModel:
    ...
    def export_data(self, data: List[RetrievalDict]) -> None:
        ...
        for each in data:
            ...
            self._db_controller.set_inventory_sold(
                each["ean13"], each["wave_serial_number"]
            )
        ...
        df = pd.DataFrame(
            {
                "名称": [each["name"] for each in data],
                "品牌": [each["brand"] for each in data],
                "价格": [each["price"] for each in data],
                "波次名称": [
                    self._db_controller.convert_wave_serial_number_to_wave_name(
                        each["wave_serial_number"]
                    )
                    for each in data
                ],
