...
from src.common.database.query_filter import AttrFilter, GroupByFilter, TimeFilter, TimeFilterEnum
...
class ChartPluginController:
    ...
    def get_count_groupby_brand(self, is_sold: Literal[0, 1] = 0
                                , statistic_time: Optional[TimeFilterEnum] = None,
                                start_time: Optional[date] = None,
                                end_time: Optional[date] = None) -> List[Tuple[str, int]]:
        """获取品牌数量"""
        count = func.count(1)
        ...
        if statistic_time is not None or start_time is not None or end_time is not None:
            query = TimeFilter.inventory_created_time(query,
