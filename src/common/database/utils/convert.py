...
def convert_wave_serial_number_to_wave_name(wave_serial_number: str) -> str:
    ...
    return convert_batch_serial_number_to_datetime(wave_serial_number).strftime(f'%Y年%m月第{batch_number:03d}批')
...
def convert_id_to_ean13(item_id: int) -> str:
    """将数字转换为EAN13条形码"""
    return convert_length12str_to_ean13(str(item_id).zfill(12))
