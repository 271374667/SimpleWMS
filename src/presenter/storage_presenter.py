...
class StoragePresenter:
    def __init__(self):
        ...
        self._table_handler = TableHandler(
            self.get_view().get_table_widget(), StorageDict
        )
        self.get_view().get_display_lcd().display(
            self.get_model().get_newest_batch_number()
        )
    ...
    def _on_confirm_button_clicked(self) -> None:
        ...
        if not item_name:
            ...
            ui.show_warning_infobar(
                title="衣服名称为空！", content="请重新输入衣服名称"
            )
        ...
        if not price or price == 0:
            ...
            ui.show_warning_infobar(
                title="价格不能为零或者空！", content="请重新输入价格"
            )
        ...
        if not auto_switch:
            ...
            for _ in range(quantity):
                new_data: StorageDict = {
                    ...
                    "brand": brand,
                    "price": price,
                    "batch_serial_number": self.get_model().gen_batch_serial_number(
                        batch
                    ),
                }
        ...
        self._table_handler.add_rows(new_data_list)
        loguru.logger.debug(
            f"添加了一行数据:{item_name} {brand} {price} {batch_serial_number} {quantity}"
        )
    ...
    def _on_return_mode_switch_btn_clicked(self) -> None:
        ...
        if current_mode is True and self.get_view().show_mask_dialog(
            ...
            self.get_view().show_success_infobar(
                title="切换模式成功!", content="当前模式已经切换为退货模式"
            )
        ...
        if current_mode is False:
            self.get_view().show_info_infobar(
                title="取消切换模式", content="您已经取消了切换模式的操作"
            )
    ...
    def _on_return_lineedit_enter(self) -> None:
        ...
        if not current_text:
            ui.show_warning_infobar(
                title="输入内容错误", content="您当前没有输入任何内容,请检查您的输入"
            )
            return

        # 检查是否是数字
        if not current_text.isdigit():
            self.get_view().show_warning_infobar(
                title="输入类型错误", content="输入必须为数字"
            )
            return
        ...
        if not self.get_model().is_real_ean13(current_text):
            ui.show_warning_infobar(
                title="输入格式错误", content="您输入的不是标准的EAN13,请检查您的输入"
            )
            return
        ...
        if not self.get_model().is_inventory_sold(current_text):
            ui.show_warning_infobar(
                title="输入商品出错",
                content="您输入的商品未出库或者不存在,请检查您的输入",
            )
            return
    ...
    def _delete_current_row(self) -> None:
        ...
        if current_row == -1:
            ui.show_warning_infobar(
                title="没有选中任何行！", content="请选中要删除的行"
            )
            return
    ...
    def _export_normal_data_to_excel(self) -> None:
        ...
        self.get_view().show_state_tooltip("正在导出数据", "请稍等...")
    ...
    def _export_return_data_to_excel(self) -> None:
        ...
        def finish_func():
            ...
            self.get_view().show_success_infobar(
                title="修改成功！",
                content=f"{len(data)}条数据已经成功同步至数据库中!",
                duration=-1,
            )
            self._table_handler.clear()
    ...
    def _connect_signals(self) -> None:
        ...
        ui.get_return_switch_button().checkedChanged.connect(
            self._on_return_mode_switch_btn_clicked
        )
        ...
        ui.get_batch_spinbox().valueChanged.connect(
            self._on_batch_spinbox_value_changed
        )
